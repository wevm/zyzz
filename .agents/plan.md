...
- [x] Replace the direction helpers with the accepted `where` tagged template, lowering each interpolated compound inside `:where()` (nesting through functional pseudo-classes), joining the template text, and validating the result with the CSS parser: selectors without `&`, unknown pseudo-classes, nested `:has()`, `:visited` inside `:has()`, and interpolations inside quoted or bracketed text are diagnostics.
- [x] Make `css` definitions the relationship identity and remove `ref`, the `Marker` runtime, and marker contracts. `Css.compile` emits one identity class per definition even when its declarations are fully shared, deduplicated, or absent. `where` interpolations resolve module-level constants, `namespace` members, object members, and packed imports; compiled definitions publish as `style` contract entries (version 13) with namespace members. State is selector text over real or `data-*` attributes. Source, consumer-type, packed-contract, and browser fixtures cover combinators, negation, pseudo-elements, escapes, nested keys, namespace members, forward references, re-exports, and Chromium matching.
