...
- [ ] Revise relationship conditions to the accepted positional overloads `helper(ref, state?)` and `helper(ref, pseudo, state?)`. Type the pseudo as `` `:${string}` `` and validate it with the CSS parser in `Markers`, rejecting unknown pseudo-classes, pseudo-elements, top-level combinators, `&`, selector lists, and nested `:has()`; fold `has` into the pseudo string and remove the closed `Pseudo` union and its duplicate runtime regex. Reject `:has()` and `:visited` for `descendant`, `siblingAfter`, and `anySibling` as compiler diagnostics. Drop `has`/`pseudo` from reserved schema names. Move the affected `.test-d.ts` expectations to source fixtures, cover both overloads through variables and packed contracts, add a nested-relationship browser case, and update the `zyzz/web` API docs and guides.
