...
Fixtures live in `src/vite/Solid.test.ts`, `src/vite/Svelte.test.ts`, and `src/ConfigOutput.test.ts`; React hydration checks use esbuild-compiled output and plain HTML fixtures verify serialized attributes and client updates. [Compatibility](../docs/introduction/compatibility.md) claims React, Solid, Svelte, and plain HTML fixtures only.
...
- [ ] Record supported framework/build-tool versions and add concise setup examples. Claim support only after the corresponding consumer gate passes. [Solid](../docs/introduction/solid.md) and [Svelte](../docs/introduction/svelte.md) setup pages record pinned fixture versions; Vue and Next.js pages remain previews.
...
- [x] Keep the numbered capability union and usage snippets synchronized as features land. Map 01–07 to source/value/theme work, 08–09 to recipes, 10–14 to selectors/queries/animation, 15–16 to contributions, 17–19 to renderer/library/external contracts, 20–22 to the explicit backlog, and 23 to native. Items span phases where stated; external-CSS examples do not satisfy typed API gates. Superseded: [parity.md](parity.md) is frozen as the 2026-09-08 audit, and [capabilities.md](capabilities.md) plus this plan record current support.
...
- [x] Define keyframe offsets, comma-separated stops, declaration-only frame bodies, importance rejection, stable imported references, theme variables, conditional reachability, and animation shorthand/list parsing. Test sampled animation progress and reduced motion in a real browser; measure reused/distinct animations and emitted CSS/JavaScript size. Covered by `Transform.keyframeAcceptance.test.ts`, `Host.keyframeAcceptance.test.ts`, the motion/range browser fixtures, and the at-rule benchmark lanes (#101, #107).
...
- [x] Implement callable theme handles retaining metadata and `.className`; return generated scope props with optional `colorScheme`. Cover root `<html>`, nested scopes, no-option inheritance, rejected options, aliases/imports, packed handles, dynamic catalog selection, and compiler/type/browser parity. Delivered in #81 and #82; `ConfigSelection.test.ts` covers aliases, packed handles, rejected fields/schemes, and nested scheme forcing in Chromium.
...
- [ ] Measure compilation, incremental updates, type-check cost, raw/compressed CSS, callable/props-merging overhead, class-string bytes, total transfer, browser style recalculation, and native adapter cost independently. Type-check cost is measured per public call by the attest benches and the TypeScript 5.9/6.0/7.0 matrix (#111); the other measurements consolidate here as their pipelines land.
