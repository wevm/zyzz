...
| Solid                    | Merged in [#77](https://github.com/wevm/zyzz/pull/77); packed consumers and module recovery verified; benchmark harness reuse open      |
| Svelte                   | Merged in [#79](https://github.com/wevm/zyzz/pull/79); script-block authoring, packed consumers, and recovery verified; benchmarks open |
...
1. Reuse the browser render benchmark harness for Solid and Svelte; their packed-consumer, recovery, and Svelte script-block gates are closed.
...
| [#77](https://github.com/wevm/zyzz/pull/77) | Solid           | Real Solid compiler and renderer: types, SSR, hydration identity, signals, theme schemes, removed overrides, CSS edits, disposal, production assets; packed HTML-output library consumer; config module rename/remove/recreate recovery with last-good styles retained   | Benchmark harness reuse                                                             |
| [#79](https://github.com/wevm/zyzz/pull/79) | Svelte          | Svelte 5 consumer with styles in shared TypeScript modules and in component script blocks: types, SSR, hydration identity, reactive values, schemes, CSS edits, production assets, line-aligned script maps; packed HTML-output library consumer; config module recovery | Benchmark harness reuse; `svelte-check` over script-block authoring                 |
...
Fixtures live in `src/vite/Solid.test.ts`, `src/vite/Svelte.test.ts`, and `src/ConfigOutput.test.ts`, driven by `test/fixtures/Framework.ts` (install, types, SSR, Chromium, production, recovery) with the packed library from `test/fixtures/Library.ts`; React hydration checks use esbuild-compiled output and plain HTML fixtures verify serialized attributes and client updates. Svelte script blocks compile through `src/vite/internal/Svelte.ts`, a tag boundary scan that keeps script line counts so the Svelte plugin's folded source map stays line-accurate. [Compatibility](../docs/introduction/compatibility.md) claims React, Solid, Svelte, and plain HTML fixtures only.
...
2. **Solid** (merged, #77; packed and recovery fixtures added; benchmark gate open): integrate the existing TSX/Vite path with normal `class` and dash-separated inline style keys. Verify signal-driven updates, dynamic variable bindings, theme/scheme changes, SSR, hydration, and supported refresh behavior.
...
- [ ] Verify mount/update/unmount, computed styles, development CSS updates, production CSS loading, source maps, and add/edit/remove/rename recovery. Require SSR/hydration identity for runtimes that implement hydration. For plain DOM/HTML, verify server attribute serialization and element/style identity across client updates; no hydration lifecycle is required. Solid and Svelte cover SSR/hydration, CSS edits, production loading, class-literal source maps, and config module rename/remove/recreate recovery with failed rebuilds keeping the last good styles; Vue and Next.js remain open.
- [ ] Exercise packed style/theme consumers and confirm no framework dependency enters core or unrelated renderer output. Packed HTML-output selection is covered in `ConfigSelection.test.ts`; packed Solid and Svelte consumers render, hydrate, and build from an installed `@acme/theme` tarball, and the package manifest is checked for framework dependencies. Vue and Next.js remain open.
- [ ] Record supported framework/build-tool versions and add concise setup examples. Claim support only after the corresponding consumer gate passes. [Solid](../docs/introduction/solid.md) and [Svelte](../docs/introduction/svelte.md) setup pages record pinned fixture versions and the Svelte script-block contract; Vue and Next.js pages remain previews.
...
- [ ] Document nearest eligible container selection, explicit containment, named raw queries, and stylesheet-level rule boundaries. Implement stylesheet contributions through `fontFace`, `global`, `keyframes`, and `layers`, with optional reset and authored layer placement. The contribution helpers, optional reset, and layer placement are merged (#71, #86); the container-selection documentation remains.
...
- [ ] Merge layer order constraints with stable topological ordering and canonical-name tie breaking; report contradictory cycles at their sources. Emit one initial order prelude, preserve dotted hierarchy and authored rule order, and define stable cross-module ordering independently of traversal or chunk completion. Verify normal/unlayered precedence, important reversal, repeated authored rules, and external stylesheet load-order limits in a real browser. Topological merging, the single prelude, dotted hierarchy, and source-attributed conflict diagnostics are implemented in `web/internal/Contributions.ts`; Chromium fixtures cover global layers and reset ordering. Important reversal, repeated authored rules, and external load-order limits lack browser evidence.
...
- [ ] Publish CSS plus layer/contribution metadata for libraries, with namespaced public layers and application-owned top-level ordering. Verify packed-library and plain-CSS consumption, metadata conflicts, shared initial stylesheet ownership, and explicit native rejection. Benchmark project-wide discovery, incremental collection, ordering, and full CSS/JS/metadata delivery against matched existing-library workloads where equivalent. Packed layer metadata, conflicts, reset ownership, and Chromium consumption are covered in `Graph.stylesheets.test.ts` (#86); plain-CSS consumption, native rejection, and matched delivery benchmarks remain open.
...
- [ ] **2.5f — Full Acceptance:** require every rule, descriptor, nested form, and supported context to have type/extraction/emission/map evidence, packed-library and watch coverage, and applicable real-browser fixtures. Record experimental/legacy browser availability separately. Add source-owned diagnostics for unsupported native semantics and benchmark compiler/output changes. No runtime authoring validation. The compiler obligations pass under `check:at-rules:full`; `check:at-rules:rendering`, `check:at-rules:targets`, and `check:at-rules:legacy-full` still fail on open renderer and target reviews.
