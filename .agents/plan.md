...
- [x] Verify web configuration-to-browser theme selection, stable component classes, nested scopes, forced/system schemes, and packed handles through the theme-selection browser fixtures. Native table selection and expanded switch-timing workloads remain separate gates.
...
- [x] Verify `theme.vars` inference, unknown paths, incompatible domains, absence of implicit root theme data, bundled/custom themes, inherited overrides, light/dark fallbacks, variable liveness, and removal of authoring callbacks from generated modules.
