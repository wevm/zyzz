...
| 3.8   | `feat: compile native style tables`               | Universal authoring contracts and the complete static native inventory; pure tables, explicit value semantics, shared static variants, composition, and StyleSheet helper parity. See 3.8a–3.8c.                                                                   | Every static native capability is expressible and typed; shared semantics and explicit target overrides follow the universal parity matrix. No silent omission or unbounded Cartesian products; renderer acceptance remains required.          |
...
| ----- | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| 3.8a  | Pin the inventory and specify universal authoring, target resolution, overrides, and incompatible value semantics. | Reviewed contracts and a CI matrix that exposes every remaining gap.                                        |
| 3.8b  | Extend shared definitions and native compilation across the full static native property/value inventory.           | Typed native outputs and faithful values; preserve all existing web behavior.                               |
| 3.8c  | Complete universal static variants, composition, and StyleSheet helper equivalents/interoperability.               | Public and packed consumers share inference, defaults, null handling, ordering, and native array semantics. |
