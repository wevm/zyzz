...
The Phase 2 backlog slice after #111 lands first-appearance layer tie breaking with Chromium precedence fixtures, property-mapping benchmark lanes, container-selection documentation, a plain-CSS packed-library consumption fixture with delivery sizes, and the Chromium target review of the at-rule matrix. Next steps: the remaining 32 unreviewed Chromium at-rule targets, namespaced public layers, native rejection, and the open stylesheet items below.
...
Bring renderer output and framework source support forward from Phase 4 into Phase 2. This work started after the runtime benchmark work in [PR #74](https://github.com/wevm/zyzz/pull/74), preserving [PR #73](https://github.com/wevm/zyzz/pull/73)'s benchmark priority. Complete these integrations before resuming the remaining Phase 2 feature backlog.

Delivered so far, as a stack on main:

| PR                                          | Scope           | Covered                                                                                                                                             | Open gates                                                                          |
| ------------------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| [#76](https://github.com/wevm/zyzz/pull/76) | Renderer output | `output: 'html'` selected once in `Config.create`; linked compiler execution; consumer types; React and plain DOM browser coverage                  | Packed HTML-output consumers beyond the selection fixtures                          |
| [#77](https://github.com/wevm/zyzz/pull/77) | Solid           | Real Solid compiler and renderer: types, SSR, hydration identity, signals, theme schemes, removed overrides, CSS edits, disposal, production assets | Packed-library consumers; add/edit/remove/rename recovery; benchmark harness reuse  |
| [#79](https://github.com/wevm/zyzz/pull/79) | Svelte          | Svelte 5 consumer with styles in shared TypeScript modules: types, SSR, hydration identity, reactive values, schemes, CSS edits, production assets  | Component script-block authoring; packed-library consumers; recovery; benchmarks    |
| [#78](https://github.com/wevm/zyzz/pull/78) | Vue             | Draft closed without merge                                                                                                                          | Entire Vue gate, including SFC script-block authoring and virtual-module identities |
| —                                           | Next.js         | Documentation preview only                                                                                                                          | Entire Next.js gate below                                                           |

Fixtures live in `src/vite/Solid.test.ts`, `src/vite/Svelte.test.ts`, `src/ConfigOutput.test.ts` (linked HTML-output compilation), and `src/runtime/Html.test.ts`, where React hydration checks use esbuild-compiled output and plain HTML fixtures verify serialized attributes and client updates in Chromium. [Compatibility](../docs/introduction/compatibility.md) claims React, Solid, Svelte, and plain HTML fixtures only.
...
1. **Renderer output** (merged, #76): define typed output for DOM `class`, `className`, inline style objects, and serialized style attributes. Preserve classes, CSS variables, theme scopes, owned data attributes, escaping, units, existing class/style override merging, and removal of stale values. Verify React and plain DOM consumers through the shared compiler.
2. **Solid** (merged, #77; recovery and packed gates open): integrate the existing TSX/Vite path with normal `class` and dash-separated inline style keys. Verify signal-driven updates, dynamic variable bindings, theme/scheme changes, SSR, hydration, and supported refresh behavior.
3. **Vue** (deferred; draft #78 closed): support both imported styles from separate TypeScript modules and authoring in Vue single-file component script blocks. Handle SFC/virtual-module identities, source maps, dependency edits, and normal template class/style bindings without passing template syntax into core.
4. **Svelte** (merged for imported modules, #79; script-block authoring open): support imported TypeScript style modules and authoring in Svelte component script blocks. Preserve normal template class/style bindings, reactive updates, source maps, dependency edits, SSR, hydration, and development refresh through the shared compiler.
5. **Next.js** (not started): bring the existing integration and both bundler acceptance gates into Phase 2. Verify the application build and server-rendering paths independently of React renderer support.
...
For each integration (checked only when every initial integration passes; per-framework state is in the table above):

- [ ] Add real consumer type fixtures and browser integration coverage for static styles, dynamic bindings, existing class/style override merging, themes, and color schemes. Extend these fixtures with `cx` composition and variants when their Phase 3 APIs land; neither blocks Phase 2 integrations. Done for React, plain HTML, Solid, and Svelte; open for Vue and Next.js.
- [ ] Verify mount/update/unmount, computed styles, development CSS updates, production CSS loading, source maps, and add/edit/remove/rename recovery. Require SSR/hydration identity for runtimes that implement hydration. For plain DOM/HTML, verify server attribute serialization and element/style identity across client updates; no hydration lifecycle is required. Solid and Svelte cover SSR/hydration, CSS edits, and production loading; add/remove/rename recovery remains open for both.
- [ ] Exercise packed style/theme consumers and confirm no framework dependency enters core or unrelated renderer output. Packed HTML-output selection is covered in `ConfigSelection.test.ts`; packed Solid and Svelte consumers remain open.
- [ ] Record supported framework/build-tool versions and add concise setup examples. Claim support only after the corresponding consumer gate passes. The [Solid](../docs/introduction/solid.md) setup page records its pinned fixture versions; the [Svelte](../docs/introduction/svelte.md) page records none yet (they live only in `src/vite/Svelte.test.ts`); Vue and Next.js pages remain previews.
- [ ] Reuse the browser benchmark harness for relevant renderer/adapter changes, with matched baselines within each framework and untimed correctness checks. Include required helpers and emitted bytes; React results do not prove non-React performance. Only React is measured today.
...
Phase 1 is merged: typed definitions, literal CSS emission, source extraction, module rewriting, file hosts, and portability coverage. Phase 2 includes merged in-memory themes, token-name authoring, and relative source graph linking; incremental graph compilation, Vite 8 integration, standalone Lightning CSS processing, and lazy physical modules are merged. Packed themes (#23), configuration core contracts (#24), and configuration source integration (#27) are merged. The 2.3 property batches (#28–#61) and the consolidated conformance PR (#63) map all 670 pinned properties. The Phase 2 feature stack (#64–#71, #81–#90), the framework stack (#76, #77, #79), the at-rule stack (#95–#101, #107), and type benches (#111) are merged. Later Phase 2 acceptance gates remain open where marked below.
...
- [ ] Extend host integration to virtual/framework sources, library contracts, and Next.js using native host facilities. Do not add a general package resolver or watcher to the styling compiler. Solid and Svelte sources compile through the Vite adapter (#77, #79); Vue SFC virtual modules and Next.js remain open.
...
6. **2.4b — Selectors and Conditions:** pseudos/elements, data/ARIA states, the accepted typed ref/ancestor API and group/peer/descendant relations, nesting, media/supports/container conditions, and `@starting-style`. Specify helper specificity separately from raw selectors and retain browser behavior.
...
- [x] Keep the numbered capability union and usage snippets synchronized as features land. Map 01–07 to source/value/theme work, 08–09 to recipes, 10–14 to selectors/queries/animation, 15–16 to contributions, 17–19 to renderer/library/external contracts, 20–22 to the explicit backlog, and 23 to native. Items span phases where stated; external-CSS examples do not satisfy typed API gates. Superseded: [parity.md](parity.md) is frozen as the 2026-09-08 audit, and [capabilities.md](capabilities.md) plus this plan record current support; the inventory's Deferred Surface table is refreshed to the merged state.
- [x] Include Panda in the same union. Cover 24 multipart component styling through separate element definitions and the accepted ref API, 25 semantic token dependency/conditional-token design after source identity, and 26 responsive recipe selections alongside recipe composition. Keep each recipe's output a single props object. Sections 24–26 exist in the archived union; their implementation gates live under Phase 3 and the explicit CSS backlog.
...
- [ ] Add `zyzz/themes/default` with named `css`, `theme`, and raw `tokens` exports; add bound `variants` when recipe compilation lands in Phase 3. Bundle colors, typography, spacing, radii, and related scales using the ordinary theme contract; keep light/dark values within the theme. The dataset, `theme`, `css`, and `tokens` exist in `src/themes/default.ts` and are exercised by `Theme.queries.test.ts`; the package entrypoint is intentionally unpublished until variants land.
...
- [x] Implement the accepted `ref(schema?)`, `ancestor(ref, condition?)`, and the matching descendant/sibling helpers from architecture. Infer finite state keys/values from the ref alone, autocomplete simple pseudos, and validate relative `has` selectors. Generated private data attributes retain source/package identity, avoid recipe collisions, and never require runtime DOM lookup or CSS generation.
- [x] Verify ref state-name lowering rejects ASCII-case-fold collisions (`state`/`State`) and applies the same lowercase fragments in HTML attributes and selectors; typed selections remain case-sensitive.
...
- [x] Document nearest eligible container selection, explicit containment, named raw queries, and stylesheet-level rule boundaries. Implement stylesheet contributions through `fontFace`, `global`, `keyframes`, and `layers`, with optional reset and authored layer placement. Evidence: `docs/guides/conditions.md#container-selection` and `docs/api/core/Theme/README.md` describe unnamed aliases compiling to `@container (width >= …)` against the nearest ancestor with eligible containment, named aliases selecting the nearest ancestor carrying that `containerName` with `containerNames` validating alias names only, raw parenthesized queries passing through with any name, application-owned `containerType`/`containerName` containment, and the boundary between conditions nested in module class rules and `global` rules in the shared stylesheet; the compiled forms were confirmed against `Graph.compile` output. The contribution helpers, optional reset, and authored `within`/`@layer` placement are implemented and covered by the checked stylesheet items below.
...
- [ ] Define keyframe offsets, comma-separated stops, declaration-only frame bodies, importance rejection, stable imported references, theme variables, conditional reachability, and animation shorthand/list parsing. Test sampled animation progress and reduced motion in a real browser; measure reused/distinct animations and emitted CSS/JavaScript size. Grammar, packed references, and stop validation are covered by `Transform.keyframeAcceptance.test.ts` and `Host.keyframeAcceptance.test.ts` (#101, #107), and the contributions browser fixture samples animated opacity. A real-browser reduced-motion check and reused-versus-distinct animation CSS/JavaScript measurements remain.
- [ ] Specify scoped/global contribution identity and explicit in-memory ownership, multi-source fonts and URLs, layer nesting/order/importance, and tree-shaking side effects. Global external names and contract-only theme interoperability require a documented decision before claiming parity. Ownership, multi-source fonts, nesting, and tree-shaking are implemented (#71, #86); the external-name and theme-interoperability decision is still undocumented.
...
- [x] Merge layer order constraints with stable topological ordering and first-appearance tie breaking; report contradictory cycles at their sources. Emit one initial order prelude, preserve dotted hierarchy and authored rule order, and define stable cross-module ordering independently of traversal or chunk completion. Verify normal/unlayered precedence, important reversal, repeated authored rules, and external stylesheet load-order limits in a real browser. Evidence: `src/web/internal/Contributions.ts` `order()` selects the first-discovered available name at each topological step, so unconstrained names keep authored order (`layers(['reset','base']); layers(['components'])` emits `@layer reset,base,components;`) and the compiler never sorts consumer layers; `Graph` visits modules by sorted identity, so the prelude does not depend on object insertion order or chunk completion. `Transform.contributions.test.ts` verifies the merged constrained order across reversed calls (including dotted `theme.light`/`theme.dark`) with the unconstrained `utilities` layer keeping its authored position, identical preludes across reversed module insertion order, the source-attributed `Conflicting layer order constraints.` diagnostics for Transform and packed owners, and checks in Chromium that a later layer wins normal declarations (including a layer declared in a later call), unlayered rules beat layered ones, `!important` reverses to the earlier layer over both layers and unlayered rules, repeated authored rules resolve last-wins within one layer, and an external `@layer components,base;` parsed before the compiled prelude fixes the order (documented in `docs/guides/stylesheets.md` and `docs/api/web/layers.md`).
...
- [ ] Publish CSS plus layer/contribution metadata for libraries, with namespaced public layers and application-owned top-level ordering. Verify packed-library and plain-CSS consumption, metadata conflicts, shared initial stylesheet ownership, and explicit native rejection. Benchmark project-wide discovery, incremental collection, ordering, and full CSS/JS/metadata delivery against matched existing-library workloads where equivalent. Progress: `Graph.stylesheets.test.ts` now loads a compiled library's published stylesheet (shared prelude, layered globals, keyframes, theme scope, and class rules) and its bundled packed classes in Chromium beside independently authored CSS from an application that never runs the compiler; the application's unlayered rule beats the library's layered rule regardless of order, its own `@layer` statement decides whether `app` or the library's `components` layer wins, and the packed scope switches colors through `color-scheme` alone. Packed consumption, metadata conflicts, and shared reset ownership are covered by the existing packed-library, conflicting-contribution, and reset-ordering fixtures. `Transform.contributions.bench.ts` records delivery for the shared graph workload: shared CSS 99/109/83, module CSS 34/54/38, JavaScript 117/124/103, and contract metadata 690/344/290 raw/gzip/Brotli bytes, byte-identical against `b6398e7`; two sequential matched pairs measured 2.34 ms ±6.53% / 2.70 ms ±9.60% before and 2.94 ms ±19.26% / 2.59 ms ±7.77% after, with overlapping uncertainty. Still open: namespaced public layer names, explicit native rejection (no native adapter exists yet), and discovery/incremental collection benchmarks against matched external workloads.
...
- [ ] Implement callable theme handles retaining metadata and `.className`; return generated scope props with optional `colorScheme`. Cover root `<html>`, nested scopes, no-option inheritance, rejected options, aliases/imports, packed handles, dynamic catalog selection, and compiler/type/browser parity. Named catalog selection through `themes({ theme, colorScheme })` is delivered in #81 and #82, with `ConfigSelection.test.ts` covering aliases, packed handles, rejected fields/schemes, and nested scheme forcing in Chromium. Callable single-theme handles (`theme(options)` retaining `.className`) remain unimplemented; see [Theme Application](../docs/api/core/Theme/apply.md).
...
- [x] Add dedicated property-mapping benchmark lanes recording compilation and emitted sizes. Evidence: `src/ConfigMappings.bench.ts` compiles the shared `test/fixtures/Mappings.ts` configuration as a packed library and measures consumer transforms with 10/100 additional aliased declarations (`px`, `paddingX!`, `space`, plus a literal longhand), writing minified CSS, bundled JavaScript, and map sizes under ignored `bench/results/mappings/`. Two sequential matched pairs against the branch point `b6398e7` on the same machine (Node 24, Vitest 4.1.9, 30 minimum iterations, 500 ms warmup): 10-style means 5.94 ms ±7.97% / 5.65 ms ±9.84% before and 4.05 ms ±4.76% / 4.54 ms ±7.14% after; 100-style means 25.10 ms ±8.74% / 28.05 ms ±8.55% before and 24.89 ms ±6.34% / 25.31 ms ±6.62% after; the unchanged 100-style configuration graph control measured 25.68 ms ±11.52% / 22.62 ms ±9.39% before and 19.49 ms ±9.44% / 17.49 ms ±8.49% after. The candidate changes no mapping code path, the baseline always ran second, and the control moved by a similar margin, so no speed change is established. Emitted sizes are byte-identical: 10 styles deliver 5,184/322/262 raw/gzip/Brotli CSS bytes and 1,363/556/491 JavaScript bytes (878 gzip total); 100 styles deliver 43,614/1,004/666 CSS and 6,733/1,407/1,013 JavaScript bytes (2,411 gzip total). Reproduce with `pnpm exec vp test bench src/ConfigMappings.bench.ts --run --no-file-parallelism --outputJson bench/results/mappings.json`.
...
- [x] **2.5a — Inventory and Context Contracts** (#95, #107): pin the full MDN at-rule/descriptor inventory, including nested page/font rules and alternate forms. Set a 100% inventory-accounting gate immediately; keep implementation coverage separate and require 100% before claiming full support. Finalize ordered conditional/layered helper contexts, external names, query/profile references, and CSS-function signatures.
- [x] **2.5b — Nested Rules and Existing Helpers** (#96, #107): complete `@scope`, all media/supports/container forms including scroll-state queries, legal starting-style/layer contexts, all font-face descriptors, and named timeline-range keyframe stops. Preserve variant/compound inference and CSS nesting semantics. Variant inference is deferred with Phase 3.
- [x] **2.5c — Named Declarations** (#97): add direct `counterStyle`, `positionTry`, `fontPaletteValues`, and `colorProfile` functions. Preserve domain-specific references through imports, aliases, re-exports, lists/shorthands, packed contracts, and rebuilds. Reuse variable registration from `Vars.define` without duplicating its API.
- [x] **2.5d — Document Declarations** (#99): add `page`, all page-margin boxes, `fontFeatureValues` and its nested blocks, and `viewTransition`. Cover repeated calls, conditional/layered placement, eagerness, and CSS order.
- [x] **2.5e — Statements and CSS Functions** (#100, #107): add `importCss`, `namespace`, `customMedia`, and `cssFunction`; implement the output charset policy and explicit legacy `@document` support. Preserve namespace boundaries, relative URLs, import ordering/conditions, and CSS function parameter/result domains. Legacy `@document` is accepted as a grouping key under `global`.
- [ ] **2.5f — Full Acceptance:** require every rule, descriptor, nested form, and supported context to have type/extraction/emission/map evidence, packed-library and watch coverage, and applicable real-browser fixtures. Record experimental/legacy browser availability separately. Add source-owned diagnostics for unsupported native semantics and benchmark compiler/output changes. No runtime authoring validation. Progress: the compiler gate passes for 22/22 rules and 62/62 descriptor entries. The Chromium target review in `test/conformance/at-rule-matrix.json` now records `native` compatibility with `partial` rendering for 25 entries backed by existing asserting fixtures (`@container`, `@counter-style` system/symbols/suffix, `@font-face` font-family/src, `@font-palette-values` and its three descriptors, `@function`/`result`, `@keyframes`, `@layer`, `@media`, `@page`/`size`, `@position-try`, `@property` and its three descriptors, `@scope`). `pnpm check:at-rules:targets` still fails on 32 Chromium entries without an asserting fixture: seven `@counter-style` descriptors, eleven `@font-face` descriptors, `@font-feature-values` with seven nested entries, `@page/page-orientation`, `@custom-media`, `@document`, `@import`, `@starting-style`, and `@supports`. See the [acceptance review](../test/conformance/at-rule-acceptance.md#chromium-target-review).
...
- [ ] Measure compilation, incremental updates, type-check cost, raw/compressed CSS, callable/props-merging overhead, class-string bytes, total transfer, browser style recalculation, and native adapter cost independently. Per-call type instantiation cost is measured by the attest benches under the TypeScript 5.9 and 6.0 lanes, and the 7.0 lane reports whole-program `tsc` diagnostics only (#111); the other measurements consolidate here as their pipelines land.
...
- [ ] PR 1.3–1.5: add cold-process source builds, warm builds, unchanged edits, new styles, removed styles, and imported-dependency edits. Include parsing, scanning, rewriting, and output writing explicitly. In-memory emission must remain a separate measurement. `Host.bench.ts` covers cold, unchanged, and edit-to-artifact rebuilds and `Graph.bench.ts` covers incremental consumer edits; explicit per-stage breakdowns and added/removed-style lanes remain open.
