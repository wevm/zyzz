...
| --- | --- | --- |
| [#81](https://github.com/wevm/zyzz/pull/81) | Callable named-theme selection | Previous head passed CI; selector review fix needs current-head CI. |
| [#82](https://github.com/wevm/zyzz/pull/82) | Initialization and hydration | Previous head passed verification and benchmarks; reviewed contract fixes need current-head CI. |
| [#83](https://github.com/wevm/zyzz/pull/83) | Property mappings and spacing tokens | Packed identity, extension, and mixed-target regressions pass locally; current-head CI required. |
| [#85](https://github.com/wevm/zyzz/pull/85) | Typed relationships | Browser suite passed; prior run timed out in three unrelated TypeScript subprocesses. Current-head CI required. |
| [#86](https://github.com/wevm/zyzz/pull/86) | Stylesheet completion | Clean/repeated builds and four shared-CSS regressions pass after packaging fixes. Final browser/CI run required. |
| [#87](https://github.com/wevm/zyzz/pull/87) | Registered variables and extraction | Packed/default/type integrations pass locally. Verification and benchmarks are running. |
...
- [x] Implement the pure `Config.create` factory and inferred `css`/layer contracts with inline or reusable themes, complete named alternatives, explicit defaults, and isolated normalization. Source extraction, `vars`, and `variants` remain separate follow-ups.
- [x] Compile named configuration instances through the source graph: direct bound calls, static theme members, immutable aliases/re-exports, config edits, and versioned packed declarations. Retain token/layer inference; layer bodies, `vars`, and variants remain deferred.
- [ ] Add pure `Config.create` from the root namespace while retaining reusable `Theme.define`/`Theme.extend`. Export `const { css, variants, theme } = Config.create(...)` from `zyzz.config.ts`, without a default export. Consumers import the bound helpers directly. Support inline or defined single themes, mixed named catalogs, mutually exclusive `theme`/`themes`, inferred required named defaults, token-free omission, and explicit layer lists. Return bound `css`/`variants` plus the corresponding normalized theme handle(s), without requiring a layer-reference object.
- [ ] Preserve inference and extraction through destructured config exports (`css`, `variants`, `theme`, and `themes`), including CSS variable references; cover aliases, re-exports, source edits, and packed exports. Integrations must follow these bindings to the originating config without requiring a default export or runtime configuration execution.
- [x] Expose named-theme selection as `themes({ theme: 'mint', colorScheme: 'dark' })`, inferring the required theme name from catalog keys. Keep default token references on `theme`, permit typed runtime selection, and verify nested scopes and scheme inheritance without property-access selection. Callable selection is implemented alongside destructured `css` and `theme` exports.
