...
- [ ] Replace the direction helpers with the accepted `where` tagged template. Resolve ref handles and static ref applications as interpolations in `Markers`, lower each to a `:where()`-wrapped attribute compound, join with the template text, and validate the result with the CSS parser, rejecting non-ref interpolations, selectors without `&`, nested `:has()`, and `:visited` inside `:has()`. Remove `ancestor`, `descendant`, `siblingBefore`, `siblingAfter`, `anySibling`, the `Pseudo` union, the direction lowering table, and the `has`/`pseudo` schema reservations. Brand the ref application return type so hand-written attribute objects fail as interpolations. Cover presence, state, multi-ref, selector-list, nested-key, and packed-contract fixtures in source, consumer-type, and browser tests, and update the `zyzz/web` API docs and guides.
