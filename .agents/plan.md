...
- [x] Replace the direction helpers with the accepted `where` tagged template. `Markers` resolves ref handles and static ref applications as interpolations, lowers each to a `:where()`-wrapped attribute compound (nesting through functional pseudo-classes), joins the template text, and validates the result with the CSS parser, rejecting non-ref interpolations, selectors without `&`, unknown pseudo-classes, nested `:has()`, and `:visited` inside `:has()`. The direction helpers, `Pseudo` union, direction lowering table, and `has`/`pseudo` schema reservations are removed; ref applications carry a brand so hand-written attribute objects fail as interpolations. Source, consumer-type, packed-contract, and browser fixtures cover presence, states, multiple refs, selector lists, parent distance, and nested keys.
