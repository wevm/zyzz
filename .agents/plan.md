...
Additional agreed APIs are callable static `css(style)`, `cx(...)`, `Vars.define`/`Vars.set`, optional `ClassName<Properties>` contracts, and `Css.fontFace`/`Css.global`/`Css.keyframes`/`Css.layers`. Export `css` and `variants` directly and bind both on themes. Extract recipe props with `NonNullable<Parameters<typeof button>[0]>`; no variant namespace or props helper is needed.
...
7. **2.4c — Stylesheet Contributions:** module-level `Css.layers` and `Css.global` with project-wide collection and hoisting; keyframes, font faces, and opt-in reset. Implement typed layer placement, source identity, ordering constraints, shared initial CSS, and browser/packaging gates. Variants remain Phase 3.
...
- [ ] Document nearest eligible container selection, explicit containment, named raw queries, and stylesheet-level rule boundaries. Implement stylesheet contributions through `Css.fontFace`, `Css.global`, `Css.keyframes`, and `Css.layers`, with optional reset and authored layer placement.
...
- [ ] Implement the accepted [layer/global collection API](architecture.md#layer-and-global-collection): `Css.layers(names)` returns inferred readonly at-rule references, and `Css.global(styles)` accepts selectors and supported nested rules anywhere at module scope. Allow layer references in `css`/`theme.css`; keep unwrapped rules unlayered and remove the consumer-facing compiler layer configuration. Prove computed-key property/value/token inference, imported references, invalid names, and unknown reference properties.
