...
The runtime 3.4 slice composes known dynamic style/recipe applications and bounded conditional presence cases. Repeated recipe ownership clears stale attributes and private slots; different recipe owners cannot claim the same attribute. HTML composition retains canonical inputs without runtime CSS parsing. The following binding slice supports immutable local props variables and const aliases without repeating their initializers. Escaping/mutated values are rejected. Ternaries, arbitrary external props, packed definitions, and bound conditional composition results remain open.

The web framework acceptance slice exercises recipes and composition through Solid/Svelte SSR, hydration, reactive payload removal, responsive compounds, source edits, disposal, and production builds. Packed Next.js Webpack/Turbopack consumers exercise the React path. These checks do not complete the matched framework benchmark matrix or native acceptance.

| Order | Proposed title                                   | Scope                                                                                                                                                                                                                                                 | Acceptance before merge                                                                                                                                                                                                                        |
| ----- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.1   | `feat: compile single-element variants`          | Root and theme/config-bound recipes; base, finite string/boolean axes, defaults, null suppression, array compound matches, and styling overrides. One element and one props object; no slots.                                                         | Public `Parameters` inference and rejected inputs; real source compilation and browser selection; stable identity, normalized attributes, ordered axes/compounds, and unchanged rule counts across updates.                                    |
...
Status: in progress; root and bound static recipes implemented, remaining acceptance open.
