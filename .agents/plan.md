...
| Next.js                  | `zyzz(nextConfig)` implemented in this branch; webpack and Turbopack fixtures pass; Pages Router, packed consumers, and React `style` typing open |
...
2. Reopen Vue on the merged renderer output, and close the Next.js follow-ups: Pages Router, project-wide contribution discovery, relative assets, packed consumers, and the React `style` prop contract.
...
| this branch                                 | Next.js         | `zyzz(nextConfig)` with one loader for webpack and Turbopack: Server/client components, streaming, hydration identity, navigation, config edits, Fast Refresh, recovery | Pages Router; project-wide contribution discovery; relative assets; packed consumers; React `style` typing |
...
5. **Next.js** (implemented in this branch as `zyzz(nextConfig)` with one loader for both bundlers; see the evidence below): bring the existing integration and both bundler acceptance gates into Phase 2. Verify the application build and server-rendering paths independently of React renderer support.
...
Next priority: close the open Solid/Svelte gates and reopen Vue under the [Framework Integration Priority](#framework-integration-priority), then resume the remaining feature backlog. Solid, Svelte, and Next.js (webpack and Turbopack) fixtures pass; Vue SFCs, per-integration packed-consumer fixtures, and the React `style` prop contract remain open.
...
- [ ] Extend host integration to virtual/framework sources and library contracts using native host facilities. Do not add a general package resolver or watcher to the styling compiler. Solid and Svelte sources compile through the Vite adapter (#77, #79); Next.js uses the webpack and Turbopack loader runners for resolution, dependency tracking, and CSS delivery; Vue SFC virtual modules remain open.
