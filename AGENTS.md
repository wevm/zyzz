...
- Run benchmark measurements, benchmark fixture checks, and type-instantiation benchmarks only in the Benchmarks workflow, except the local-only suite under `bench/native/`. Keep ordinary test discovery outside `bench/`; framework correctness tests must not perform comparison builds or write benchmark results.
...
- Measure public type instantiation costs with `@ark/attest`: import `bench` from `@ark/attest` in colocated `*.bench-d.ts` fixtures, declare the public values from type-only entrypoint imports, and snapshot each body inline with `.types([count, 'instantiations'])`. Keep one exported module-scope `baseline` function that warms shared contracts without repeating a bench expression. `pnpm bench:types` runs every fixture against the installed TypeScript and fails past the 20% threshold; `pnpm update:types` rewrites the inline baselines after an intentional contract change. TypeScript 7 checks and builds every workspace. API-based tests, conformance scripts, and attest use the separate `typescript-api` dependency on TypeScript 6. CI runs type-instantiation fixtures with that JavaScript compiler and reports whole-program diagnostics from TypeScript 7.
...
- Group string selectors under `selectors` inside `style`. Interpolate previously declared definitions directly, such as `selectors: { [`${card}:hover &`]: { color: 'blue' } }`. Use `style()` for identity-only styles; apply state through ordinary data/ARIA attributes. Preserve authored CSS specificity.
- Export the `style` and `variants` leaf functions directly and bind both on themes; conceptual modules use namespace exports. Infer variant props with standard `Parameters`, without a variant namespace.
- Every `style` definition is callable and returns props when applied. Callbacks receive only typed runtime values; no context helpers. Use the suffix ` !important` for importance, arrays for fallbacks, and `theme.tokens`/`theme.vars` for references. Calls consume declared values and merge only styling overrides (`className`/`style`/`variables` on web). Other component props stay on the component; unknown inputs are errors. Dynamic variant choices take typed callbacks and scoped payload selections; compound matches use choice names, while values bind to precompiled slots. Spread applied props; `cx` preserves bindings and recipe attributes. Generated functions never create CSS rules.
