...
- Recommend named `style` and `variants` imports from `zyzz` or `zyzz.config.ts`. Config modules export bound helpers from a local `Config.create(...)` instance. Group related definitions in a plain `const styles = { ... }` object. Static styles are values applied with `style={styles.button}`; dynamic styles and variants remain callable and their results also go in `style`. Keep preview notes for deferred capabilities. No callback grouping API.
...
- Canonical static authoring uses named `style` imports and the native JSX `style` prop. Resolve opaque values at compiled intrinsic elements, preserving custom-component forwarding, ordinary inline styles, attribute evaluation order, and class merging. Keep legacy `css` callables for compatibility. Dynamic callbacks and variants use this same value transport when their planned compiler stages land; never generate CSS rules at runtime.
