...
- Recommend named imports from `zyzz.config.ts`, such as `{ css, variables }` or `{ variants }`. Export bound helpers and alias `theme.vars` as `variables`; retain a default config export for integrations. Keep ordinary source component imports; generated modules belong to standalone build/distribution documentation, not introductory application examples.
