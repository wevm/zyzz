...
- Import `where` directly from `zyzz/web`, alongside `global` and `fontFace`. Any module-level `css` definition serves as an element identity; compose relationship keys by interpolating definitions into the `where` tagged template as computed style keys. Express state with real or `data-*` attributes in the selector text.
