...
- Import `ancestor`, `anySibling`, `descendant`, `marker`, `siblingAfter`, and `siblingBefore` directly from `zyzz/web`, alongside `global` and `fontFace`. Define markers at module scope, apply their callables to the related elements, and use relationship helpers as computed style keys.
