...
- Group related `css` and `variants` definitions in `const styles = { ... }`, using descriptive keys and `styles.card()` or `styles.button(options)` at application sites. Use this convention throughout README, docs, architecture examples, and application fixtures. Export `styles` from shared style modules. Keep focused API parameter snippets and fixtures testing other supported syntax when that syntax is the subject of the example or test. Overview usage should show two styles applied to nested elements in a component.
...
- Keep pull request titles and bodies short and concrete: a few sentences explaining the problem and resulting behavior, one small example, and one validation line. Every PR body must include an example: a usage snippet, before/after behavior, or a relevant command for tooling, tests, and documentation changes. Link detailed reports instead of copying logs, test inventories, or benchmark methodology. Omit product framing.
