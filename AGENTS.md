...
- Group related `css` and `variants` definitions in `namespace style {}`, with exported `const` members and `style.card()` or `style.button(options)` at application sites. Use this convention throughout README, docs, architecture examples, and application fixtures. Use `export namespace style` in shared style modules. Keep reusable literal declarations inside the namespace and reference earlier declarations directly. Namespaces belong at module scope and require TypeScript lowering. Keep focused API parameter snippets and fixtures testing other supported syntax when that syntax is the subject of the example or test. Overview usage should show two styles applied to nested elements in a component.
