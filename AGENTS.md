...
- Group string selectors under `selectors` inside `css`. Interpolate previously declared definitions directly, such as `selectors: { [`${card}:hover &`]: { color: 'blue' } }`. Use `css()` for identity-only styles; apply state through ordinary data/ARIA attributes. Preserve authored CSS specificity.
...
- Declare independent CSS variables with `variable(kind, options)` from `zyzz`; group related declarations in `namespace variables`. Use the `variables` style property for static assignments and `reference.set(value)` for typed inline assignments. Computed object keys lose individual domain information in TypeScript; document this limitation.
