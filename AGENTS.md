...
- Import `where` from `zyzz` and use tagged templates as computed `css` keys. Interpolate previously declared `css` definitions directly, such as `[where`${card}:hover &`]`. Use `css({})` for identity-only styles; apply state through ordinary data/ARIA attributes. Preserve authored CSS specificity.
