  ...
  for (const kind of Runtime.cases)
    for (const repeat of [1, 2])
      describe(`runtime comparison / ${count} styles / ${kind} / repeat ${repeat}`, () => {
        ...
        const libraries = repeat === 1 ? Runtime.libraries : [...Runtime.libraries].reverse()
          ...
          bench(library, () => {
            ...
            setup: async () => {
              ...
              if (!output) {
                ...
                const directory = Path.resolve('bench/results/runtime', String(count), kind)
                ...
                await Fs.writeFile(Path.join(directory, `${library}.json`), JSON.stringify({
                  count,
                  css,
                  javascript,
                  kind,
                ...
                await Fs.writeFile(Path.join(directory, `${library}.css`), output.css)
                await Fs.writeFile(Path.join(directory, `${library}.js`), output.javascript)
              }
              apply = output.apply
            },
            teardown: () => {
              ...
              if (!results.some((props) => props.className.length > 0))
                throw new Error('Runtime benchmark produced no observable props.')
              results.length = 0
            },
          })
        }
