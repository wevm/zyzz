...
Compiler-generated callbacks use `Dynamic` from `zyzz/runtime`. Applications normally author callbacks with `style` and use the resulting `style.Dynamic<values>` callable.

## create

`Dynamic.create(options)` accepts `create.Options` and returns `style.Dynamic<Record<string, string | number>>`. `options.className` is the complete generated class list. `options.slots` maps every required input name to a fixed custom-property `name` and scalar `type`. An optional `zero` flag permits numeric zero alongside string dimensions. The compiler supplies these fields.
