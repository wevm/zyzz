...
The defaults compile `src` into `dist`, with adjacent module CSS, `zyzz.shared.css` for shared contributions, and `zyzz.css` combining both in dependency order for applications that load one stylesheet. Treat `dist` as compiler output, not an application import convention. Downstream tooling consumes the rewritten tree and lowers TypeScript/JSX. Original relative imports remain authored normally; the build selects its input root.
