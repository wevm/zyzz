...
Layer order merges the constraints from every call. Names left unconstrained keep the order in which they first appear, so `layers(['reset', 'base'])` followed by `layers(['components'])` emits `@layer reset,base,components;`. Modules contribute in sorted identity order, independently of chunk completion. Declare a constraint wherever relative order matters.
