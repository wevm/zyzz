  ...
  return (
    <main {...styles.page()}>
      <div {...styles.content()}>
        <header>
          <h1>Zyzz examples</h1>
          <p>React + compiler API</p>
          <p {...styles.muted()}>
            <code>scripts/build.ts</code> compiled this source tree into{' '}
            <code>.zyzz</code> with <code>Host</code> from{' '}
            <code>zyzz/node</code>, then bundled it through Vite's JavaScript
            API.
          </p>
          <div {...styles.row()}>
            <button
