...
`zyzz/metro` compiles `Styles.ts` during bundling with the requested iOS or Android platform. The app imports that source module directly. Metro tracks edits to imported style and theme modules. A React provider connects `useColorScheme` and explicit overrides to compiled components. A local resolver maps NodeNext `.js` imports to TypeScript sources. No separate generation command or application style artifacts are required.
