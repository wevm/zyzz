...
export function theme(
  ...
  if (!metadata?.contract.variableSet)
    throw new Variables.InvalidError([], 'Expected a variable set.')
...
export function from(metadata: Token.Metadata): Variables.Definition {
  return buildTree(metadata.values, metadata.contract) as Variables.Definition
}
...
export function build(
  ...
  function visit(input: unknown, path: string[]) {
    if (
      ...
      (input &&
        typeof input === 'object' &&
        ('default' in input || 'light' in input || 'dark' in input))
    ) {
      if (!path.length)
        throw new Variables.InvalidError(path, 'Expected a variable record.')
      const value = read(input, path)
      const name = path.join('.')
      if (base && !Object.hasOwn(base, name))
        throw new Variables.InvalidError(
          path,
    ...
    if (!entries.length && !base)
      throw new Variables.InvalidError(
        path,
        'Variable records cannot be empty.',
      )
  ...
  visit(input, [])
  return buildTree(Object.freeze(values), contract) as Variables.Definition
}

function buildTree(values: Token.Metadata['values'], contract: Token.Contract) {
  ...
  Object.defineProperty(tree, Token.definition, {
    value: Object.freeze({ contract, values }),
  })
  freeze(tree)
  return tree
}
...
function record(input: unknown, path: string[]) {
  if (
    ...
    throw new Variables.InvalidError(path, 'Expected a plain variable record.')
  return Reflect.ownKeys(input).map((key) => {
    ...
    if (
      ...
      throw new Variables.InvalidError(
        path,
        'Variables require enumerable data properties.',
      )
...
function read(
  ...
  if (
    ...
    if (domain(light) !== 'color' || domain(dark) !== 'color')
      throw new Variables.InvalidError(path, 'Color schemes require colors.')
...
export function mappings(input: unknown): Variables.Mappings | undefined {
