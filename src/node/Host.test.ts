...
test('file builds preserve working artifacts, cache unchanged inputs, and clean owned outputs', async () => {
  ...
  try {
    ...
    await Fs.writeFile(Path.join(outDir, 'ignored.ts'), 'invalid output source')
    ...
    const before = await Fs.readFile(Path.join(outDir, 'button.ts.css'), 'utf8')
    await Fs.writeFile(
    ...
    await host.build()
    await Fs.rename(Path.join(root, 'button.ts'), Path.join(root, 'renamed.ts'))
    expect((await host.build()).changed).toMatchInlineSnapshot(`
      [
        "button.ts",
        "button.ts.css",
...
test('ownership survives reopening and refuses modified files and symlink outputs', async () => {
  const root = await Fs.mkdtemp(Path.join(project, '.fixture-host-ownership-'))
  const outDir = Path.join(root, 'output')
  ...
  try {
    ...
    await Fs.writeFile(Path.join(outDir, 'button.ts.css'), 'edited by consumer')
    await expect(host.build()).rejects.toThrowErrorMatchingInlineSnapshot(
...
test('real recursive watching handles nested additions, failures, recovery, and disposal', async () => {
  ...
  try {
    ...
    if (!('error' in failed) || !(failed.error instanceof Source.ExtractError))
    ...
    await next((event) => 'result' in event && event.result.files.length === 0)
    await host.close()
    await expect(host.build()).rejects.toThrowErrorMatchingInlineSnapshot(
