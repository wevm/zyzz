...
export const button=variants({variants:{intent:{primary:{color:'brand'},quiet:{color:'red'}}},defaultVariants:{intent:'primary'}});`
  ...
  test('preserves configured aliases through source and packed contracts', async () => {
    ...
    expect(result.modules['app.ts']!.css).toMatchInlineSnapshot(
      ...
      .z-pl-8px-1i1s57j183c9bx-0{padding-left:8px;}
      .z-pr-8px-1i1s57j183c9bx-1{padding-right:8px;}
      .z-text-1i1s57j183c9bx-2{&:where([data-intent="primary"]){color:var(--z-tu8smm21l81sow-variants-color_2e_brand,#06c);}}
      .z-text-1i1s57j183c9bx-3{&:where([data-intent="quiet"]){color:black;}}"
    `,
    )
    expect(
  ...
  test('supports local theme recipes and destructured aliases', async () => {
    ...
    expect(module.exports.a()).toMatchInlineSnapshot(`
      {
        "className": "z-text-1e8a67zly006l z-style-1e8a67z1uaws1j-117",
      }
    `)
    expect(module.exports.b()).toMatchInlineSnapshot(`
      {
        "className": "z-text-1e8a67zly006l z-style-1e8a67z1uaws1j-172",
      }
    `)
  })
})
