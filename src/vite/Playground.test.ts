/** Exercises the runnable React playground in Vite development and production. @module */
...
describe('zyzz', () => {
  test('renders the React playground through its consumer config', async () => {
    const directory = await Fs.mkdtemp(Path.resolve('.fixture-playground-'))
    ...
    try {
      // The example consumes real package exports, including the built Vite plugin.
      await Util.promisify(ChildProcess.execFile)('pnpm', ['build'], {
        timeout: 60000,
      })
      ...
      for (const mode of ['development', 'production']) {
        const server = await (async () => {
          ...
          const development = await Vite.createServer({
            ...config,
          ...
          return {
            ...
            url: development.resolvedUrls!.local[0]!,
          }
        })()
        ...
        try {
          ...
          await page
            .getByRole('checkbox', { name: 'Highlight children' })
            .check()
