...
These samples have 1.6–5.0% relative error and do not establish a general speed advantage. The comparison predates merging the newer namespace-style changes from main. Reproduce with `vp test bench bench/Compilation.bench.ts src/compiler/Transform.fontPalette.bench.ts --run --no-file-parallelism -t 'unique.*(zyzz|tailwind)|font palette publication' --outputJson bench/results/print.json`.
