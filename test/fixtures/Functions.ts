...
export const outer=cssFunction({ '@layer functions': { '@media screen': {parameters:[{name:'--x',syntax:'<length>'}],returns:'<length>',body:{'--local':inner(),result:'var(--x)','@media (width > 1px)':{result:'var(--local)'},'@supports (width: 1px)':{result:'var(--x)'},'@container (width > 1px)':{result:'var(--local)'}}} } });
