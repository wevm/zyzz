...
export async function verify(options: verify.Options) {
  ...
  try {
    ...
    const config = `import {Config} from 'zyzz';import {theme as library} from '@acme/theme';export const {style,theme}=Config.create({cssOutput:'${cssOutput}',theme:library});`
    const files = {
  ...
  `)
    ...
    await Fs.writeFile(Path.join(app, 'app/config.ts'), changedConfig('#0a0'))
    await page.waitForFunction(
      () =>
        getComputedStyle(document.querySelector('main > h1')!).color ===
        'rgb(0, 170, 0)',
