...
export async function verify(options: verify.Options) {
  ...
  try {
    ...
    const files = {
      ...
      'app/layout.tsx': `import {vars} from '@config';export default function Layout({children}:{children:React.ReactNode}){return <html className={vars().className}><body>{children}</body></html>}`,
